class CfgPatches
{
	class RedFalcon_Apache_Heli
	{
		units[] = {""};
		weapons[] = {};
		requiredVersion = 0.1;
		requiredAddons[] = {"DZ_Vehicles_Parts","DZ_Vehicles_Wheeled","DZ_Scripts","DZ_Data","RFFSHeli_Apache_sounds","RFFSHeli_Core","RFFSHeli_Apache_Heli"};
	};
};

class CfgSlots
{
	class Slot_Apachecard
	{
		name = "Apachecard";
		displayName = "Pilot License Card";
		ghostIcon = "set:pilot_license image:pilot_license";
	};
};

class CfgVehicles
{
	class RedFalcon_Cards_Base;
	class RFFSHeli_base;

	class RFFSHeli_Apache: RFFSHeli_base
	{
		attachments[] = 
		{
			"AviationBattery",
			"AviationIgniterPlug",
			"AviationHydraulicHoses",
			"AviationWiringHarness",
			"apache_doors_driver",
			"apache_doors_codriver",
			"NivaWheel_1_1",
			"NivaWheel_1_2",
			"NivaWheel_2_1",
			"NivaWheel_2_2",
			"Reflector_1_1",
			"Flight_Case1",
			"Apachecard"
		};
		class GUIInventoryAttachmentsProps
		{
			class Engine
			{
				name = "$STR_attachment_Engine0";
				description = "";
				icon = "cat_vehicle_engine";
				attachmentSlots[] = 
				{
					"AviationBattery",
					"AviationIgniterPlug",
					"AviationHydraulicHoses",
					"AviationWiringHarness",
					"Reflector_1_1",
					"Apachecard"
				};
			};
		};
	};
	
	class RedFalcon_Cards_Apache: RedFalcon_Cards_Base
	{
    	scope = 2;
		inventorySlot[]=
		{
			"Apachecard"
		};
	};
	
};